...
*   Ця бібліотека запозичує багато функцій з фреймів даних мови R.
    *   Фрейм — це двовимірна таблиця, стовпці якої мають імена
...
*   Pandas використовує зворотну скісну риску `\`, щоб показати перенесення рядку, коли вивід занадто широкий, щоб поміститися на екран.
...
*   Насправді краще індексувати за країнами.
...
*   Зверніть увагу, що це дані, а не метод.
    *   Подібно до `math.pi`.
    *   Тому не використовуйте `()`, щоб спробувати її викликати.
...
*   Транспонування (written `.T`) не копіює дані, а лише змінює подання даних.
...
> > ~~~
> >{: .language-python}
> > Зверніть увагу, що `help(to_csv)` видає помилку! Це тонкощі, і це пов’язано з тим, що `to_csv` НЕ є функцією сама по собі
